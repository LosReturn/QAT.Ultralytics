...
from torch.ao.quantization import disable_fake_quant, enable_fake_quant, disable_observer, enable_observer
...
from utils.ax_quantizerV2 import AXQuantizer, load_config
from torch.ao.quantization.pt2e.utils import _fuse_conv_bn_
import utils.quantized_decomposed_dequantize_per_channel
import operator
def replace_aten_bn_with_cudnn_bn(model: torch.fx.GraphModule) -> torch.fx.GraphModule:
    """
    This pass replaces aten.batch_norm nodes with cudnn_batch_norm nodes.
    
    It correctly handles the difference in arguments by dropping the 9th argument
    ('cudnn_enabled') from the original call.
    """
    for node in model.graph.nodes:
        # Check if the node is the one we want to replace
        if node.op == 'call_function' and node.target == torch.ops.aten.batch_norm.default:
            
            # The original node's arguments include the extra 'cudnn_enabled' boolean
            # aten.batch_norm(input, weight, bias, mean, var, training, momentum, eps, cudnn_enabled)
            original_args = node.args
            
            # We need to pass only the first 8 arguments to cudnn_batch_norm
            new_args = original_args[:-1]
            # Insert the new node right before the old one
            with model.graph.inserting_before(node):
                # 创建新的 cudnn_batch_norm 节点
                new_cudnn_bn_node = model.graph.call_function(
                    torch.ops.aten.cudnn_batch_norm.default, 
                    args=new_args
                )
                
                # aten.cudnn_batch_norm.default 总是返回一个元组 (output, mean, invstd)。
                # 而原始的 aten.batch_norm.default 只返回 'output' 张量。
                # 因此，我们必须提取元组的第一个元素作为实际的输出。
                output_tensor_node = model.graph.call_function(
                    operator.getitem, # 使用 operator.getitem 来获取元组的第一个元素
                    args=(new_cudnn_bn_node, 0) # 获取索引为 0 的元素
                )
                
                # 将所有使用旧节点输出的地方，替换为新节点的第一个输出（即归一化后的张量）
                node.replace_all_uses_with(output_tensor_node)

            # 清理：删除旧的、现在已不再使用的节点
            model.graph.erase_node(node)
            
    # Check for any errors in the modified graph
    model.graph.lint()
    # Recompile the Python code from the modified graph
    model.recompile()
    return model
...
def train(hyp, opt, device, callbacks):
    ...
    ep = torch.export.export_for_training(deepcopy(model.cpu()), (torch.rand(2, 3, 640, 640),), dynamic_shapes=dynamic_shapes)
    ...
    # model = replace_aten_bn_with_cudnn_bn(model)
