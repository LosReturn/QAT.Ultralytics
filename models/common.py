...
def _make_grid(nx, ny, i, anchors, stride, na):
    ...
    return grid.to(d), anchor_grid.to(d)

def decode_bbox_hardcode(x,  device):
    nc = 80
    no = nc+5
    nl = 3
    na = 3
    ...
    for i, x_i_raw in enumerate(x):
        ...
        # Make grid if not already done or if shape changed
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), training_onnx=False, dnn=False, data=None, fp16=False, fuse=True):
        ...
        pt, qat , jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, paddle, triton = self._model_type(w)
        ...
        if pt:  # PyTorch
            ...
            model = torch.export.load(w).module().to(device)
        else:
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            ...
            if self.training_onnx:
                y = decode_bbox_hardcode(y,  self.device)
