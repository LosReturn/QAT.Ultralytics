...
from models.yolo import Model, QatDetectionModel
...
def train(hyp, opt, device, callbacks):
    ...
    onnx_program = torch.onnx.export(model, (inputs,), dynamo=True, training=torch.onnx._C_onnx.TrainingMode.TRAINING)
    ...
    torch.onnx.export(
        ...
        inputs,
        "./yolov5s_float.onnx",
        training=torch.onnx._C_onnx.TrainingMode.TRAINING
    )
    ...
    if opt.ptq_warmup:
        ...
        ep = torch.export.export_for_training(deepcopy(model), (torch.rand(2, 3, 640, 640).to(device),), dynamic_shapes=dynamic_shapes)
        # from IPython import embed; embed()
        prepared_model = prepare_qat_pt2e(ep.module(), quantizer)
        torch.ao.quantization.move_exported_model_to_eval(prepared_model)
        ptq_model = prepared_model
        torch.ao.quantization.allow_exported_model_train_eval(ptq_model)
    ...
    prepared_model = prepare_qat_pt2e(ep.module(), quantizer)
    torch.ao.quantization.move_exported_model_to_eval(prepared_model)
    # save_pt = torch.export.export(prepared_model, (inputs,))
    # torch.export.save(save_pt, "./qat_yolov5s.pt")
    ...
    if RANK in {-1, 0} and ptq_model is not None:
        qat_model.model = ptq_model
        ptq_model.eval()
        model.eval()
        ptq_model.apply(disable_fake_quant)
        ptq_model.apply(disable_observer)
        LOGGER.info(f"Float results")
        results, maps, _ = validate.run(
            data_dict,
            batch_size=128 // WORLD_SIZE * 2,
            imgsz=imgsz,
            half=amp,
            model=qat_model,
        ...
        LOGGER.info(f"PTQ warmup: use 1000")
        ptq_model.apply(enable_observer)
        with torch.no_grad():
            ...
            for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
                ...
                ptq_model(imgs)  # forward

                if i*train_loader.batch_size >= 100:
                    break

            assert len(ptq_model.graph.nodes) == len(model.graph.nodes)
            for node_ptq, node_qat in zip(ptq_model.graph.nodes, model.graph.nodes):
                if node_ptq.op == "call_module" and node_ptq.target.startswith("activation_post_process"):
                    ...
                    module_ptq = ptq_model.get_submodule(node_ptq.target)
                    module_qat = model.get_submodule(node_qat.target)
                    
                    scale, zero_point = module_ptq.calculate_qparams()
                    # from IPython import embed; embed()
                    module_qat.scale = scale
                    module_qat.zero_point = zero_point.type(torch.int32)
                    if not isinstance(module_ptq,_DerivedObserverOrFakeQuantize):
                        min_val = module_ptq.activation_post_process.min_val
                        max_val = module_ptq.activation_post_process.max_val
                        if isinstance(module_ptq.activation_post_process, HistogramObserver):
                            min_val, max_val = module_ptq.activation_post_process._non_linear_param_search()
                        module_qat.activation_post_process.min_val = min_val
                        module_qat.activation_post_process.max_val = max_val
            for node_qat in model.graph.nodes:
                if node_qat.op == "call_module" and node_qat.target.startswith("activation_post_process"):
                    module_qat = model.get_submodule(node_qat.target)
                    if isinstance(module_qat,_DerivedObserverOrFakeQuantize):
                        module_qat.calculate_qparams()
 
        ptq_model.apply(enable_fake_quant)
        ptq_model.apply(disable_observer)
        # mAP
        LOGGER.info(f"PTQ results")
        results, maps, _ = validate.run(
            data_dict,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0} and epoch == 0 :
            ...
            LOGGER.info(f"QAT base PTQ results - epoch0")
        ...
        if opt.image_weights:
            cw = class_weights.cpu().numpy() * (1 - maps) ** 2 / nc  # class weights
    ...
    prepared_model_copy = deepcopy(de_parallel(model))
    prepared_model_copy.eval()
    save_pt = torch.export.export(prepared_model_copy, (inputs,))
    torch.export.save(save_pt, "./qat_yolov5s.pt")
    # from IPython import embed; embed()

    quantized_model = convert_pt2e(prepared_model_copy)
    save_pt = torch.export.export(quantized_model, (inputs,))
    torch.export.save(save_pt, "./pt2e_yolov5s.pt")
    onnx_program = torch.onnx.export(quantized_model, (inputs,), dynamo=True)
    onnx_program.optimize()
    onnx_program.save("./yolov5s_qat.onnx")
...
def parse_opt(known=False):
    ...
    # 
    parser.add_argument("--ptq-warmup", action="store_true", help="Log ndjson to file")
    parser.add_argument("--lsq", action="store_true", help="Log ndjson to file")
