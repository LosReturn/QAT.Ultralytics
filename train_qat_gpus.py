...
def train(hyp, opt, device, callbacks):
    ...
    optimizer = smart_optimizer(model, opt.optimizer, hyp["lr0"]* WORLD_SIZE, hyp["momentum"], hyp["weight_decay"])
    ...
    qat_model = QatDetectionModel(
                cfg or ckpt["model"].yaml, 
                ch=3, 
                nc=nc, 
                anchors=hyp.get("anchors"),
                model=de_parallel(model) if RANK != -1 else model,  
                device=device
                )
